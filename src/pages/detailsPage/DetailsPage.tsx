  ...
  return (
    <article className="p-4 grid gap-4 md:grid-cols-[200px,1fr]">
      ...
      <div>
        ...
        {details.tagline && <p className="italic opacity-80">{details.tagline}</p>}
        <p className="mt-2">{details.overview}</p>

        {embedUrl && (
          // wenn wir einen Trailer gefunden haben, zeigen wir den Button an
          <button onClick={() => setShowTrailer(true)} className="mt-4 px-4 py-2 rounded bg-red-600 text-white">
            🎬 Trailer ansehen
          </button>
        )}
      </div>
